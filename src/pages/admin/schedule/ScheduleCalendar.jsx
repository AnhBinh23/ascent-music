...
const DAY_MAP     = [2, 3, 4, 5, 6, 7, 1]; // index → day_of_week
const START_HOUR  = 7;
const END_HOUR    = 21;
const SLOT_HEIGHT = 80; // px per hour
const SNAP_MINS   = 30; // snap to 30-minute slots
...
const ScheduleCalendar = () => {
  ...
  const [dropTarget, setDropTarget]   = useState(null); // { dayIdx, mins }
  ...
  const onDrop = async (e, dayIdx) => {
    ...
    const newEndMins  = pos.mins + duration;
    ...
    setSchedules(prev => prev.map(s =>
      s.id === id
        ? { ...s, day_of_week: newDow, time_start: newStart, time_end: newEnd }
        : s
    ));
    setDraggingId(null);
    setDropTarget(null);
              ...
              {DAYS.map((day, dayIdx) => (
                ...
                >
                  ...
                  {dropTarget?.dayIdx === dayIdx && dragData.current && (
                    <div
                      className="absolute left-0 right-0 mx-1 rounded-xl opacity-40 border-2 border-dashed border-primary-400 bg-primary-100 pointer-events-none z-10"
                      style={{
