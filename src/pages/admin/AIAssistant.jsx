...
const QUICK_QUESTIONS = [
  ...
  'Giáo viên nào dạy nhiều lớp nhất?',
  'Lớp nào có ít học viên nhất?',
  'Doanh thu tháng này so với tháng trước?',
  'Học viên nào hay vắng mặt nhất?',
];
...
const AIAssistant = () => {
  const [messages, setMessages] = useState([
    {
      role: 'ai',
      text: 'Xin chào! Tôi là trợ lý AI của Ascent Music Center. Tôi có thể giúp bạn tra cứu thông tin học viên, lịch học, học phí và phân tích dữ liệu. Hãy hỏi tôi bất cứ điều gì! 🎵',
    }
  ]);
      ...
      <div className="flex flex-col h-full" style={{ height: 'calc(100dvh - 120px)' }}>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-3 mb-4 pr-1">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'ai' && (
                <div className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center text-sm flex-shrink-0 mr-2 mt-1">
                  🤖
                </div>
              )}
        ...
        <div className="flex gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-2">
          <textarea
            ...
            onKeyDown={handleKey}
            rows={1}
            placeholder="Hỏi bất cứ điều gì về trung tâm..."
