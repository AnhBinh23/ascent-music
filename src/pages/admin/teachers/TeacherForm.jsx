...
const TeacherForm = () => {
  ...
  const handleSave = async () => {
    if (!form.name || !form.phone) {
      toast.error('Vui lòng điền họ tên và số điện thoại!'); return;
    }
    setSaving(true);
    try {
      if (isEdit) {
    ...
    <MainLayout title={isEdit ? 'Chỉnh sửa giáo viên' : 'Thêm giáo viên mới'}>
      <Card>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-2">
          ...
          <Input label="Kinh nghiệm" name="experience"
            value={form.experience || ''} onChange={handleChange} placeholder="VD: 5 năm" />
          ...
          {/* Gợi ý lương */}
          <div className="sm:col-span-2 p-3 bg-blue-50 rounded-xl">
            <p className="text-xs text-blue-700 font-medium">
              💡 Mức lương được cài đặt riêng cho từng lớp học
              (Quản lý lớp → Chỉnh sửa lớp → Lương giáo viên)
            </p>
          </div>
