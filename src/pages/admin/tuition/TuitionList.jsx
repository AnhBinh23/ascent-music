...
const TuitionList = () => {
  ...
  useEffect(() => {
    ...
    if (search) result = result.filter(d => d.studentName?.toLowerCase().includes(search.toLowerCase()));
    setFiltered(result);
  }, [search, filterStatus, data]);

  const totalRevenue = data.filter(d => d.status === 'Đã thanh toán').reduce((sum, d) => sum + d.paid, 0);
  ...
  const columns = [
    ...
    { key: 'paid', label: 'Đã thu',
      render: (val) => <span className={`font-medium ${val > 0 ? 'text-green-600' : 'text-gray-400'}`}>{val.toLocaleString('vi-VN')}đ</span>
    },
    ...
    { key: 'id', label: '',
      render: (val, row) => row.status !== 'Đã thanh toán' && (
        <Button size="sm" icon="💰" onClick={(e) => { e.stopPropagation(); handleCollect(row); }}>
          Thu tiền
        </Button>
      )
    },
          ...
          <div className="flex flex-col gap-4">
            <div className="p-4 bg-gray-50 rounded-xl">
              ...
              <p className="text-sm text-gray-600">
                Còn lại: <span className="font-medium text-red-500">{(selected.amount - selected.paid).toLocaleString('vi-VN')}đ</span>
              </p>
            </div>
