...
const QUICK_QUESTIONS = [
  ...
  'Học phí tháng này bao nhiêu?',
  'Tôi đã điểm danh bao nhiêu buổi?',
  'Giáo viên của tôi là ai?',
  'Tôi còn bao nhiêu buổi học?',
];
...
const AIChat = () => {
  const [messages, setMessages] = useState([
    {
      role: 'ai',
      text: 'Xin chào! Tôi là trợ lý AI của Ascent Music Center. Tôi có thể giúp bạn xem lịch học, học phí, điểm danh và các thông tin khác. Hãy hỏi tôi nhé! 🎵',
    }
  ]);
  ...
  const sendMessage = async (text) => {
    ...
    setLoading(true);
    try {
      const answer = await aiService.parentChat(question);
      ...
      <div className="flex flex-col" style={{ height: 'calc(100dvh - 120px)' }}>
        ...
        <div className="flex gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-2">
          <textarea
            ...
            onKeyDown={handleKey}
            rows={1}
            placeholder="Hỏi về lịch học, học phí..."
