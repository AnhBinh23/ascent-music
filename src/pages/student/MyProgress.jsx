            ...
            {logs.map(log => (
              <div key={log.id} className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                  {log.content && (<div className="p-2 bg-blue-50 rounded-xl"><p className="text-xs text-blue-600 font-medium">📚 Nội dung</p><p className="text-gray-700 mt-0.5">{log.content}</p></div>)}
                  {log.skill && (<div className="p-2 bg-green-50 rounded-xl"><p className="text-xs text-green-600 font-medium">✅ Kỹ năng</p><p className="text-gray-700 mt-0.5">{log.skill}</p></div>)}
                  {log.weakness && (<div className="p-2 bg-red-50 rounded-xl"><p className="text-xs text-red-600 font-medium">⚠️ Cần cải thiện</p><p className="text-gray-700 mt-0.5">{log.weakness}</p></div>)}
                  {log.progress && (<div className="p-2 bg-purple-50 rounded-xl"><p className="text-xs text-purple-600 font-medium">📈 Tiến độ</p><p className="text-gray-700 mt-0.5">{log.progress}</p></div>)}
                  {log.homework && (<div className="p-2 bg-orange-50 rounded-xl sm:col-span-2"><p className="text-xs text-orange-600 font-medium">📝 Bài tập về nhà</p><p className="text-gray-700 mt-0.5">{log.homework}</p></div>)}
                </div>
                {log.teacher_name && (<p className="text-xs text-gray-400 mt-2">👨‍🏫 GV: {log.teacher_name}</p>)}
              </div>
            ))}
          </div>
        )}
