...
// Lấy ngày trong tuần (2=T2...7=T7, 1=CN)
...
const ScheduleManage = () => {
  ...
  // Tab tuần: nhóm theo day_of_week
  const byDay = DAYS.map((day, i) =>
    schedules.filter(s => s.day_of_week === i + 2 || (i === 6 && s.day_of_week === 1))
  );

  // Tab ngày: lọc theo ngày được chọn
  const schedulesByDate = schedules.filter(s =>
    s.day_of_week === getDayOfWeek(selectedDate)
  );

  // Tab tháng: tạo lịch theo tháng
  const daysInMonth = getDaysInMonth(selectedMonth);

  const ScheduleCard = ({ s, j, showDelete = true }) => (
    <div className={`p-2 rounded-xl border text-xs ${COLORS[j % COLORS.length]}`}>
