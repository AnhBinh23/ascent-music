...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    pushService.isSubscribed().then(subscribed => {
      if (!subscribed) {
        pushService.subscribe(api.post.bind(api));
      }
    });
  }, [user]);
  ...
  const sendMessage = async () => {
    ...
    setInputValue(''); // ← clear ngay lập tức
    try {
      await api.post('/messages', { to_id: contactId, message: text });
      await loadMessages(contactId);
    } catch (err) {
      setInputValue(text); // ← restore nếu lỗi
      ...
      <div className="flex flex-col bg-gray-50" style={{ height: '100dvh' }}>
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100 sticky top-0 z-10">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs" style={{ color: color(selected.role).bg }}>
